...
# raw_fcs_dir = wdir
# output_fcs_dir = output_fcs_dir
# markers_edit_csv="markers_edit.csv"
...
#      m=2
      ...
      #  if(keyval[[n]]=="CD45") {key_rename_id<-c(key_rename_id,n)}
 ...
 #     keyval[189]
    ...
    #(exprs_add[,c(add_para_id[i_add]:ncol(exprs_add))])
    #colnames(exprs_add)
    #head(add_data)
    #head(indata)
    ...
    for (j_add in 1:length(add_para_id)) {
      ...
      # pd_add <- rbind(pd_add[c(1:(j_add+add_para_id[j_add]-2)),], 
      #                 t(plist),
      #                 pd_add[c((j_add+add_para_id[j_add]-1):nrow(pd_add)),])
    ...
    #   str(params)
  #  str(out_frame@exprs[1,])
  #  names(out_frame@parameters@data$name)
    ...
    #markers_edit[del_para_id,1]

    pd_del_rownames<-row.names(pd_del)
    row.names(pd_del)<-paste0("$P",c(1:length(pd_del_rownames)))
    #names(params@data$name)<-paste0("$P",c(1:nrow(pd_del_rownames)))#important!
    #pd_del$name
    #pd_del$desc
    ...
    #attr(exprs_del,ranges)<-pd_del$range
    ...
    #write.csv((as.data.frame(keyval_del)),"keyval2.csv",row.names = F,col.names = F)
 ...
 #   str(keyval)
  #  pData(params_del)
    ...
    # keyval_del$`$TOT`<- nrow(exprs)        
    # keyval_del$`$COM`<- "Exported by R"   
    
     
     
     
 # out_frame <- flowFrame(exprs = exprs_del, parameters = params,
 #                           description = keyval)
    # keyval <- keyword(raw_flowframe)
 ...
 # pd <- pData(params)
 # keyval <- keyword(raw_flowframe)
 # out_frame <- flowFrame(exprs = exprs, parameters = params,
 #                        description = keyval) 
...
#    str(params)
#    length(out_frame@description)
#   length(raw_flowframe@description)
#   head(exprs_del)
#  out_frame@description$'$P1S'


suppressWarnings(write.FCS(out_frame, paste0(output_fcs_dir, "/",raw_file_name)))

#read.FCS( paste0(output_fcs_dir, "/",raw_file_name))
#setwd(output_fcs_dir)
#suppressWarnings(write.FCS(out_frame, raw_file_name))
