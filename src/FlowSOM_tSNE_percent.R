...
if(args$batch_correct){
	metadata<-data.frame("cell_id"=Names,"sample"=status)
        harmony_embeddings<-HarmonyMatrix(as.matrix(data), metadata, "sample")
        rownames(harmony_embeddings)<-Names
        colnames(harmony_embeddings)=paste("Harmony",1:ncol(harmony_embeddings),sep="_")
	data<-harmony_embeddings
	saveRDS(data,file.path(args$outdir,"harmony.rds"))
}
