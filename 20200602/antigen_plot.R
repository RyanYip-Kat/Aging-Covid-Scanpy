...
for(antigen in antigens){
        jpeg(file.path(args$outdir,paste0(antigen,"_condition_seurat.jpeg")),width=1024,height=1024)
        p=FeaturePlot(seurat,reduction="tsne",pt.size=args$pt_size,features=antigen,split.by="condition")+
                my_theme
        print(p)
        dev.off()

        jpeg(file.path(args$outdir,paste0(antigen,"_seurat.jpeg")),width=1024,height=1024)
...
DimPlot(seurat,reduction="tsne",pt.size=args$pt_size)+my_theme
dev.off()
