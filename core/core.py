...
class Model(object):
   def __init__(self,path,tcr_path=None,use_harmony=True,batch_key="status",n_top_genes=5000,outdir="./scanpy_result"):
      self._path=path
      self._outdir=outdir
   ...
   def _load_data(self):
      ...
      adata=self._drop_genes(adata)  # remove dropout features
   ...
   def _process(self,adata,remove_doublet=False):
